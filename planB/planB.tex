...
law became operational in May of 2013.  Further details of the timing and 
nature of the reform are provided in figure \ref{TEENfig:reformtiming}.
...
\section{Methodology}
\label{TEENscn:ID}
\Person take advantage of the random nature of the expansion of the 
...
Similarly, we are interested in determining the degree of heterogeneity of 
access by age groups, and look at teenagers (15-19 year olds), 20-34 year olds, 
and 35-49 year olds. Figure \ref{TEENfig:BirthDeath} describes trends in births 
for all ages, while appendix figures \ref{TEENfig:Trend1519}-%
\ref{TEENfig:Trend2034} seperate these by age groups.
...
In each case we estimate first the simple DD specification without 
...
It is important to note that these coefficients refer to the proportional 
reduction in rates of teen birth, not the absolute reduction in births.  In
appendix table \ref{TEENtab:aggregateASFR}, this is examined in terms of age
specific fertility rates (ASFR).  Here it is apparent that a 5.8\% reduction in 
rates of births can be thought of as a reduction in the ASFR of 3.389 women.
This implies that the reform reduced the number of teenagers ever giving birth
during the ages of 15-19 by approximately 4 per 1,000: an important effect when
compared to prevailing rates of teen pregnancy of 55 per 1,000 women in Chile.
...
The inclusion of time varying controls in columns 2-3 has relatively little 
effect on the estimates of the reform.  However, including controls for condom
usage does reduce estimates by an economically significant amount, particularly
among young women, moving point estimates from -0.056 to -0.046.  While this
provides some suggestion that mayors providing access to the reform may also
encourage higher rates of condom use, it is important to note that these controls
are noisy at best. As discussed further in appendix \ref{Pscn:Data}, while all
other time-varying controls are observed at the level of the municipality, condom
use must be calculated from survey data asking about condom use, and is only 
available at the level of the region, which is one administrative division 
\emph{above} municipalities.  While the inclusion of condom controls allows us
to estimate the conditional effect of the morning after pill reform once 
controlling for condom use, if other contraceptive measures which aren't 
observed have similar effects to condoms, we may be concerned that we are 
overstating the effect of the reform.  However, Ministry of Health evidence 
suggests that condom availability is one of the few programs under the 
discretion of the municipality.  Programs such as the prescription of the oral
contraceptive pill and contraceptive implants are defined at the national level
(where publicly available), or must be purchased privately through pharmacies
\citep{MINSAL2013}.
...
log odds, so the effect size is interpreted as: $1-\log(-0.468)=-0.37$) which I 
...
robustness checks which are reported in appendix tables (appendix table 
\ref{TEENtab:DeathOLS}), I find that OLS estimates using the rate of fetal death 
at the level of the municipality result in insignificant estimates for the over 
35 age group, while the pattern of other results remains unchanged. This suggests 
that the effect on the over 35 age group is quite noisy, and less robust than 
birth findings and the fetal death effects at younger ages.
...
$t$-test.  
%If \person were to correct for multiple hypothesis testing using a Bonferroni 
%correction, finding a significant result would be even less likely.

\subsection{Placebo Tests}
\label{TEENsscn:placebo}
\paragraph{Using the Pre-Reform Period as a Placebo}
...
\paragraph{Full Event Study Analysis}
While the lagged results above suggest that significant differences in rates
of pregnancy were not seen in reform and non-reform areas \emph{prior} to the
reform, the most compelling series of tests of the parallel trends assumption
examines trends over both the pre and post-reform periods for both areas.  
Formal tests of whether pregnancy rates diverge after the reform can then be 
assessed using an event study methodology.  In order to examine the parallel 
trends assumption more closely, I estimate an event study surrounding the 
...
I present event studies for 15-19 and 20-34 year olds in figures 
...
\appendix
\section*{Appendices}
\section[The Chilean Legislative Environment and Contraceptive Adoption]%
{The Chilean Legislative Environment and the Adoption of Emergency Contraception}
...
In this study I focus on the period surrounding this 2008 legislation as the 
...
or the Yuzpe regime. I am not aware of any evidence or survey data which exists 
...
the only survey data of which I am aware which asks about the Yuzpe method 
