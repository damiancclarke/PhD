...
*** (2) Birth Regressions
********************************************************************************
use "$DAT/AppendedBirthsEarly.dta", clear
**take 10% sample
set seed 2727
gen bin=runiform()
keep if bin>0.9
...
use "$DAT/AppendedBirths.dta", clear
keep if year>=2003
**take 10% sample
set seed 2727
gen bin=runiform()
...
append using "$DAT/AppendedFDeaths.dta"
tab year
drop if year==2002
keep if motherAge>15&motherAge<40
...
#delimit ;
twoway line twin year, xtitle("Year") ytitle("Proportion Twin") scheme(s1mono)
