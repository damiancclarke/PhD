\begin{chapabstract}
 ...
 of the quantity-quality (QQ) trade-off and so could contribute to explaining 
 ...
 the true QQ trade-off and discuss how to estimate the size of these bounds.
...
of an individual's family (sibling) size on his or her human capital attainment.
...
estimation of the QQ trade-off. We focus upon maternal health because no 
...
The emerging consensus in the literature using twin births to test the QQ 
...
QQ trade-off.  When ignoring these considerations we find that the effect of an 
...
the true size of the QQ trade-off. Given that we suggest that maternal health 
...
examine a number of methods to estimate plausible bounds on the QQ trade-off. 
...
Empirically, work which further probes the QQ trade-off is important,
...
methodology that we will use to examine twinning, and bound the QQ trade-off.
...
the QQ trade-off, accounting for the increase in rates of twinning by parity 
...
Table \ref{TWINtab:Lit} documents the principal studies in the QQ trade-off 
...
of $\delta$ that we believe hold in the case of twinning and the QQ equation.
...
For our main IV specification, we follow the existing literature, which follows
\citet{Blacketal2005} in defining
...
this cohort is made up of all children in the same country and birth year, 
...
%*******************************************************************************
\subsection{The twin instrument and the QQ trade-off} \label{TWINsscn:QQtwins}
...
also invest more after birth, the QQ trade-off will be under-estimated (we can 
...
the direction of the true QQ trade-off from below, as we partially correct for 
the bias due to the $twin_ju^H_{ij}$ term.

Conversely, OLS estimates are typically thought to over-estimate the true 
magnitude of the QQ trade-off.  If unobserved parental behaviours favour both
...
omitted variables have on estimates of the QQ trade-off, we turn to results 
...
evidence in favour of the QQ trade-off, despite the reasonably imprecise 
...
Similarly, effects of the QQ trade-off vary considerably depending upon a 
...
methodology to estimate bounds for the QQ trade-off.  
...
parity groups examined, the LTZ approach suggests that the QQ trade-off is 
...
(higher human capital).  The existing evidence from the QQ literature is 
...
with his or her mother's health---in both the developing world and the USA---we 
show that partially correcting for twin endogeneity is sufficient to push 
estimates of the trade-off up by about 3\%-4\% of a standard deviation, 
potentially explaining the lack of significant results in the existing 
literature.  Using partial identification to bound the effect of child quantity 
on child quality suggests that the \emph{true} effect size, once accounting for 
the entire health differential in favour of twin families, may even be as high 
as 8\% of standard deviation.

We are able to conclude that additional unexpected births do have quantitatively 
important effects on their siblings' educational outcomes.  An 8\% of a standard 
deviation increase is equivalent to an additional 0.3 years in the classroom.  
While the true effect of these 0.3 years of course depend on the quality of 
education imparted, time in school is at the very least a necessary condition for 
more general increases in learning outcomes.  The implications of these findings 
are wide-reaching, both in terms of the vindication of Beckerian theory, and 
particularly, in an applied sense, when considering human well-being in 
developing countries which are yet to fully pass through the demographic 
transition.
...
\newpage
\section*{Figures}
\input{\twinloc/figures/figures.tex}

\section*{Tables}
Begin overleaf.
\input{\twinloc/tables/tables.tex}
\clearpage
...
\phi^t/\phi^q=\frac{\partial educ/\partial bP}{\partial twin/\partial bP}\bigg|_{X}=\frac{\partial educ}{\partial twin}\bigg|_{X}=\gamma.
\end{equation}
...
\input{\twinloc/tables/appendixTables.tex}
\input{\twinloc/tables/onlineAppendix.tex}
\clearpage
\newpage

\section{Appendix Figures}
\input{\twinloc/figures/appendixFigs.tex}
