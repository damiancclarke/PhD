/* DHSPrep 2.00                  damiancclarke   	 		     yyyy-mm-dd:2012-03-22
*---|---------|---------|-----   soniabhalotra   -----|---------|---------|-----
*---|----1----|----2----|----3----|----4----|----5----|----6----|----7----|----8
*

This file generates data to use in the Twin Instrument paper. It uses as input t
he full set of publicly available DHS files up to October 2013. To replicate res
ults exactly no DHS files released after 2013 should be included. Full details r
egarding creating this paper from source including downloading and merging DHS,
creating crude data and formatting tables can be found in the README file locate
d in this directory.

This file merges all DHS BR (birth recode) and PR (household member recode) file
s to generate an output file with one line per child. Each child is linked to th
eir mother, and for those children who still live in the same household as their
mother, their educational attainment is available.
...
global TEMP   "./twins/temp"

cap mkdir $TEMP
cap mkdir $LOG
log using "$LOG/DHSPrep.txt", text replace
...
generat income="low" if inc_status=="L"
replace income="mid" if inc_status!="L"

********************************************************************************
*** (6) Keep required variables.
********************************************************************************
...
********************************************************************************
*** (7) Make Discretionary choices about missings.  Save data as working file.
********************************************************************************
...
lab dat "Pooled DHS data on mothers and children"
save "$DATA/DHS_twins", replace
log close
